...
async def resolve_subject(db: AsyncSession, user: User, requested: UUID | None) -> Profile:
    ...
    if not subject_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No authorized candidate is linked")
...
async def create_collection_source(
    ...
    existing = (await db.execute(select(CollectionSource).where(CollectionSource.base_url == base_url))).scalar_one_or_none()
    if existing:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Collection source already registered")
    ...
    db.add(row)
    await db.flush()
    await _audit(db, actor=actor, action="collection_source.created", resource_type="collection_source", resource_id=row.id, purpose="Configure authorized public intelligence source")
    await db.commit()
    return _source_out(row)


async def list_collection_sources(db: AsyncSession) -> list[CollectionSourceOut]:
    rows = (await db.execute(select(CollectionSource).order_by(CollectionSource.name))).scalars().all()
...
async def create_collection_subscription(
    ...
    if existing:
        ...
        existing.next_due_at = datetime.now(UTC) + timedelta(
            minutes=source_cfg.schedule_minutes
        )
        await db.commit()
...
async def collect_source(
    ...
    source_cfg = await db.get(CollectionSource, source_id)
    if not source_cfg or source_cfg.status != "active":
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Active collection source not found")
    if source_cfg.connector_kind != "scrapling":
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="This source requires its licensed connector")
    ...
    source_row = (await db.execute(select(Source).where(Source.url == document.url))).scalar_one_or_none()
    if not source_row:
    ...
    source_cfg.last_collected_at = now
    await db.flush()
    await _audit(db, actor=actor, action="signal.collected", resource_type="signal", resource_id=signal.id, purpose="Collect authorized public evidence", metadata={"source_id": str(source_cfg.id), "subject_id": str(payload.subject_id) if payload.subject_id else None})
    await db.commit()
...
async def intelligence_overview(db: AsyncSession, user: User) -> IntelligenceOverview:
    ...
    signals = (await db.execute(stmt.order_by(SignalEvent.observed_at.desc()).limit(1000))).scalars().all()
    ...
    for subject_id, items in grouped.items():
        ...
        presence.append(PresenceMetric(
    ...
    sources_active = int((await db.execute(select(func.count()).select_from(CollectionSource).where(CollectionSource.status == "active"))).scalar_one())
    pending_stmt = select(func.count()).select_from(StrategyVerdict).where(StrategyVerdict.status == "draft")
    if subject_filter:
        pending_stmt = pending_stmt.join(IntelligenceScenario).where(IntelligenceScenario.subject_id == subject_filter)
...
async def create_scenario(
    ...
    if effective_at > datetime.now(UTC) + timedelta(minutes=5):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="effective_at cannot be in the future")
    ...
    evidence = [
        {"signal_id": str(item.id), "url": item.url, "title": item.title, "observed_at": item.observed_at.isoformat(), "authority": (item.provenance or {}).get("authority")}
    ...
    db.add(verdict)
    await db.flush()
    await _audit(db, actor=actor, action="scenario.created", resource_type="scenario", resource_id=scenario.id, purpose="Generate aggregate campaign-planning scenario", metadata={"subject_id": str(profile.id), "cohort_size": payload.cohort.sample_size})
    await db.commit()
...
async def list_verdicts(db: AsyncSession, user: User) -> list[VerdictOut]:
    stmt = select(StrategyVerdict).join(IntelligenceScenario).order_by(StrategyVerdict.created_at.desc()).limit(100)
    if user.role not in {"admin", "superadmin"}:
        if not user.principal_id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No authorized candidate is linked")
        stmt = stmt.where(IntelligenceScenario.subject_id == user.principal_id)
...
async def decide_verdict(
    ...
    if verdict.status != "draft":
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Verdict was already reviewed")
    ...
    await _audit(db, actor=actor, action=f"verdict.{decision}", resource_type="verdict", resource_id=verdict.id, purpose="Authorized analyst review", metadata={"review_note": review_note})
    await db.flush()
    await db.commit()
